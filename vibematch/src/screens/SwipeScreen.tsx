...
export function SwipeScreen({
  ...
  const filtered = ALL_CARDS.filter((c) => {
    const wOk = c.weather.includes("any") || c.weather.includes(weather.id);
    const gOk = !context.genres?.length || c.genres.some((g) => context.genres.includes(g));
    return wOk || gOk;
  });
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", background: "linear-gradient(180deg,#0D0D18 0%,#0D0D0D 100%)" }}>
      ...
      <div style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: 14, padding: "6px 0 28px" }}>
        <button className="swipe-btn" onClick={undo} disabled={idx === 0} style={{ width: 46, height: 46, borderRadius: "50%", background: "rgba(255,255,255,.06)", border: "1.5px solid rgba(255,255,255,.15)", color: idx === 0 ? "rgba(255,255,255,.15)" : "#FBBF24", fontSize: 16, cursor: idx === 0 ? "default" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", transition: "transform .15s" }}>↩</button>
