    ...
    <div style={{ flex: 1, display: "flex", flexDirection: "column", background: "linear-gradient(180deg,#0D0D18 0%,#0D0D0D 100%)" }}>
      ...
      {!deckDone && (
        <div style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: 14, padding: "6px 0 28px" }}>
          <button aria-label="Отменить" className="swipe-btn" onClick={undo} disabled={idx === 0 && !deckDone} style={{ width: 46, height: 46, borderRadius: "50%", background: "rgba(255,255,255,.06)", border: "1.5px solid rgba(255,255,255,.15)", color: idx === 0 && !deckDone ? "rgba(255,255,255,.15)" : "#FBBF24", fontSize: 16, cursor: idx === 0 && !deckDone ? "default" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", transition: "transform .15s" }}>↩</button>
          <button aria-label="Не интересует" className="swipe-btn" onClick={() => swipe("left")} style={{ width: 56, height: 56, borderRadius: "50%", background: "rgba(239,68,68,.1)", border: "1.5px solid rgba(239,68,68,.25)", color: "#EF4444", fontSize: 20, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", transition: "transform .15s" }}>✕</button>
          <button aria-label="Случайный выбор" className="swipe-btn" onClick={onSurprise} style={{ width: 50, height: 50, borderRadius: "50%", background: "rgba(245,158,11,.1)", border: "1.5px solid rgba(245,158,11,.25)", color: "#FCD34D", fontSize: 18, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", transition: "transform .15s" }}>🎲</button>
          <button aria-label="Сохранить на потом" className="swipe-btn" onClick={() => swipe("up")} style={{ width: 50, height: 50, borderRadius: "50%", background: "rgba(167,139,250,.1)", border: "1.5px solid rgba(167,139,250,.25)", color: "#A78BFA", fontSize: 16, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", transition: "transform .15s" }}>🔖</button>
          <button aria-label="Хочу это" className="swipe-btn" onClick={() => swipe("right")} style={{ width: 56, height: 56, borderRadius: "50%", background: "rgba(34,197,94,.1)", border: "1.5px solid rgba(34,197,94,.25)", color: "#22C55E", fontSize: 20, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", transition: "transform .15s" }}>✓</button>
        </div>
      )}
    </div>
  );
